...
export class TrackAsiaService {
    ...
    private readonly REVERSE_URL = 'https://maps.track-asia.com/api/v2/reverse';
    private readonly DIRECTIONS_URL = 'https://maps.track-asia.com/api/route/v2';

    constructor() { }

    /**
   * Search for locations (geocoding with autocomplete)
   */
    async search(query: string, limit: number = 5): Promise<SearchResult[]> {
        try {
            // Using autocomplete endpoint for better suggestions
    ...
    async reverseGeocode(lng: number, lat: number): Promise<string> {
        try {
            const url = `${this.REVERSE_URL}?key=${this.apiKey}&lng=${lng}&lat=${lat}`;
            const response = await fetch(url);
    ...
    /**
     * Calculate route between two points using new TrackAsia Directions API
     */
    async getDirections(
        originLng: number,
        originLat: number,
    ...
    /**
     * Decode Google-style polyline to coordinates array
     */
